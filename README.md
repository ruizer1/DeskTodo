...
- **逾期不丢** —— 过期未完成的任务会被收进一个红色的「逾期」分组固定置顶，
  不会随着日子往前走自己消失
- **已完成自动清场** —— 完成的任务划掉留在列表里，过了它自己的到期时刻自动消失；
  也可以在设置里关掉，完成即隐藏
...
- **就地改文字** —— 点一下任务就能改，回车保存、Esc 撤销，断网也能改
- **右键删除**，点标题回到今天

托盘菜单里有「检查更新」（直接显示当前版本号）和「关于」。
...
- 认不出来的进「未分类」，等你右键指定 —— 不会替你藏起来
- **分错了右键改**，改一次永久记住
- 分类结果会记住，**软件更新不会把整理好的桌面重新打乱**
...
- **3 套配色**：蓝莓（默认）/ 薰衣草 / 暗色
- **透明度**：两个面板各自无级调节，100% 时背板彻底消失、只剩文字浮在壁纸上。
  只淡背板，文字和图标始终清晰
